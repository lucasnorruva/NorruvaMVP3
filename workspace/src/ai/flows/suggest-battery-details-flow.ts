...
const SuggestBatteryDetailsOutputSchema = z.object({
  suggestedBatteryChemistry: z.string().optional().describe("A common battery chemistry (e.g., 'Li-ion NMC', 'LFP', 'Lithium Polymer', 'NiMH', 'Lead-Acid')."),
...
If it is, suggest plausible values for ALL of the following battery details in the specified JSON structure. Be comprehensive.

- suggestedBatteryChemistry: (e.g., "Li-ion NMC", "LFP", "Lithium Polymer", "NiMH", "Lead-Acid")
- suggestedBatteryPassportId: (e.g., a plausible ID like "BATT-ID-EVMODULE-XYZ123", "BPID-TOOL-MAKITA-001")
...
- suggestedStateOfHealth: An object with 'value' (e.g., for a new product, 100; for refurbished, 95), 'unit' (typically "%"), and 'measurementDate' (e.g., current date in "YYYY-MM-DD" format like "2024-08-15").
- suggestedBatteryRegulationVcId: (e.g., a plausible ID like "vc:battery:acme:evmodule123", "vc:battery:powertools:drill456")
...
If the product is battery-related, attempt to provide a suggestion for ALL fields.
