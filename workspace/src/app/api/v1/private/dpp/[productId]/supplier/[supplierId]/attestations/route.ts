...
import { MOCK_DPPS, MOCK_SUPPLIERS } from '@/data'; // Import MOCK_DPPS and MOCK_SUPPLIERS
...
export async function GET(
  ...
  // Check if product and supplier exist in mock data
  const productExists = MOCK_DPPS.some(dpp => dpp.id === productId);
  const supplierExists = MOCK_SUPPLIERS.some(sup => sup.id === supplierId);
  ...
  if (!supplierExists) {
    return NextResponse.json(
      { error: { code: 404, message: `Supplier with ID ${supplierId} not found.` } },
      { status: 404 }
    );
  }
