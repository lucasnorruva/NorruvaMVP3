        ...
        <section>
          <h4 className="font-semibold mb-1">Conceptual Backend Interaction</h4>
            <ol className="list-decimal list-inside text-sm space-y-1 text-muted-foreground">
              <li>Backend validates `productId` and the requested `status` from the payload.</li>
              <li>Conceptually, it interacts with a smart contract (e.g., `DPPRegistry.sol`).</li>
              <li>Calls a function like: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">dppRegistryContract.updateDPPOnChainStatus(productId, newStatus)</code>.</li>
              <li>The smart contract would verify permissions (e.g., only owner or authorized admin).</li>
              <li>Upon success, the smart contract emits an event: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">DPPOnChainStatusUpdated(productId, oldStatus, newStatus, blockTimestamp)</code>.</li>
              <li>The mock API updates `product.metadata.onChainStatus` and adds a `LifecycleEvent` to `MOCK_DPPS`.</li>
            </ol>
        </section>
        <section>
    ...
    <Card className="shadow-lg mt-6">
      ...
      <CardContent className="space-y-4">
        ...
        <section>
          <h4 className="font-semibold mb-1">Conceptual Backend Interaction</h4>
            <ol className="list-decimal list-inside text-sm space-y-1 text-muted-foreground">
              <li>Backend validates `productId` and `lifecycleStage`.</li>
              <li>Conceptually calls a smart contract function: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">dppRegistryContract.updateDPPLifecycleStage(productId, newStage)</code>.</li>
              <li>Smart contract verifies permissions and business logic (e.g., valid stage transition).</li>
              <li>Emits event: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">DPPLifecycleStageUpdated(productId, oldStage, newStage, blockTimestamp)</code>.</li>
              <li>Mock API updates `product.metadata.onChainLifecycleStage` and adds a `LifecycleEvent`.</li>
            </ol>
        </section>
        <section>
          <h4 className="font-semibold mb-1">Example Response (Success 200 OK)</h4>
      ...
      <CardContent className="space-y-4">
        ...
        <section>
         ...
         <section>
          <h4 className="font-semibold mb-1">Conceptual Backend Interaction</h4>
            <ol className="list-decimal list-inside text-sm space-y-1 text-muted-foreground">
              <li>Backend validates `productId`, `eventDescription`, and `severity`.</li>
              <li>The `eventDescription` might be hashed (e.g., Keccak256) to get an `eventHash` for on-chain storage.</li>
              <li>Conceptually calls smart contract: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">dppRegistryContract.logCriticalDPPEvent(productId, eventHash, severity)</code>.</li>
              <li>Smart contract emits event: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">DPPCriticalEventLogged(productId, eventHash, severity, blockTimestamp)</code>.</li>
              <li>Mock API adds a `LifecycleEvent` with type "CriticalEventLogged" to `MOCK_DPPS`.</li>
            </ol>
        </section>
        <section>
          <h4 className="font-semibold mb-1">Example Response (Success 200 OK)</h4>
    ...
    <Card className="shadow-lg mt-6">
      ...
      <CardContent className="space-y-4">
        ...
        <section>
          <h4 className="font-semibold mb-1">Conceptual Backend Interaction</h4>
            <ol className="list-decimal list-inside text-sm space-y-1 text-muted-foreground">
              <li>Backend validates `productId`, `vcId`, and `vcHash`.</li>
              <li>Conceptually calls smart contract: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">dppRegistryContract.registerDPPVC(productId, vcId, vcHash)</code>.</li>
              <li>Smart contract emits event: <code className="bg-muted px-1 py-0.5 rounded-sm font-mono text-xs">DPPVCRegistered(productId, vcId, vcHash, blockTimestamp)</code>.</li>
              <li>Mock API adds a `LifecycleEvent` with type "VCHashRegistered" to `MOCK_DPPS`.</li>
            </ol>
        </section>
        <section>
          <h4 className="font-semibold mb-1">Example Response (Success 200 OK)</h4>
