...
import { Menu, UserCircle, Users, LogOut, Settings as SettingsIcon, Bell } from "lucide-react";
...
export default function AppHeader() {
  ...
  const displayedRole = useMemo(() => {
    ...
    // Check if the path starts with /app/(app)/<role>-dashboard
    if (pathSegments.length >= 3 && pathSegments[1] === '(app)') {
      const roleCandidate = pathSegments[2].replace('-dashboard', '');
      if (availableRoles.includes(roleCandidate as UserRole)) {
        return roleCandidate as UserRole;
      }
    }
    return currentRole; // Fallback to context role if path doesn't match a role dashboard
  }, [pathname, currentRole, availableRoles]);

  // Effect to synchronize RoleContext with URL-derived role
  useEffect(() => {
    if (displayedRole !== currentRole) {
      setCurrentRole(displayedRole);
    }
  }, [displayedRole, currentRole, setCurrentRole]);

  useEffect(() => {
    setNotifications(generateMockNotifications(displayedRole));
  }, [displayedRole]);


  const handleRoleChange = (newRole: UserRole) => {
    // setCurrentRole(newRole); // Context will be updated by the useEffect watching pathname
    router.push(`/${newRole}-dashboard`);
  };

  const handleLogout = () => {
    alert("Mock Logout: User logged out!");
    // router.push('/login'); // Example redirect after logout
  };

  const currentDashboardPath = `/${displayedRole}-dashboard`;
    ...
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between gap-4 border-b border-border bg-card px-4 backdrop-blur-md md:px-6">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        <div className="flex items-center gap-2">
          ...
          <Select value={displayedRole} onValueChange={(value) => handleRoleChange(value as UserRole)}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-80 sm:w-96">
            <DropdownMenuLabel className="flex justify-between items-center">
              <span>Notifications ({displayedRole.charAt(0).toUpperCase() + displayedRole.slice(1)})</span>
              {notifications.length > 0 && (
